...
import com.delaroystudios.fragrancecart.data.Shoe;
...
public class ShoeAdapter extends RecyclerView.Adapter<ShoeAdapter.ViewHolder> {
    ...
    public ShoeAdapter(Activity mContext, Cursor cursor) {
        dataCursor = cursor;
        context = mContext;
    }

    @Override
    public ShoeAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(ShoeAdapter.ViewHolder holder, int position) {


        dataCursor.moveToPosition(position);

        id = dataCursor.getInt(dataCursor.getColumnIndex(FragranceContract.FragranceEntry._ID));
        String mName = dataCursor.getString(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOENAME));
        String mDescription = dataCursor.getString(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOEDESCRIPTION));
        String mImageUrl = dataCursor.getString(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOEIMAGE));
        Double mPrice = dataCursor.getDouble(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOEPRICE));
        String mSize = dataCursor.getString(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOESIZE));
        int mUserrating = dataCursor.getInt(dataCursor.getColumnIndex(FragranceContract.FragranceEntry.COLUMN_MENSHOEUSERRATING));
